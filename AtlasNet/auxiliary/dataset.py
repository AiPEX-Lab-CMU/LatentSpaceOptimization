...
class Ellipsoid(data.Dataset):
    def __init__(self, rootpc = "/home/workstation1/Documents/James_C/LatentSpaceOptimization/AtlasNet/data/ellipsoid_points/",train = True):
        ...
        self.shuffled_points = []

        if train:
            for i in range(self.train_len):
                self.shuffled_points.append(np.asarray(self.fns_pc[i]))
        else:
            for i in range(self.train_len,len(self.fns_pc)):
                self.shuffled_points.append(self.fns_pc)

        self.ValueMeter = AverageValueMeter()


    def __getitem__(self, index):
        name = self.fns_pc[index]
        pts_file = self.rootpc + name
        with open(pts_file,'rb') as f:
            points = pickle.load(f)
        point_set = torch.from_numpy(points)
    ...
    def __len__(self):
        if self.train:
            ...
            test_len = int(len(self.fns_pc)) - self.train_len
            return test_len
